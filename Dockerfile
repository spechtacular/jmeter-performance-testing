...
RUN apt-get -y update \
  ...
  && nuget update -self \
  && apt-get clean
# download the taurus code from https://github.com/Blazemeter/taurus to the local filesystem 
# so the taurus-master/bzt/resources/chrome_launcher.sh path is correct or change the path
COPY taurus-master/bzt/resources/chrome_launcher.sh /tmp
